...
const TimelineCard = ({ interaction, index }) => {
  ...
  return (
    ...
    <div key={index} className="bg-white p-4 rounded shadow mb-4 flex items-center justify-between hover:bg-green-100 gap-4 hover:scale-102  transition duration-200">
      ...
      <div className="flex items-center justify-center h-20 w-20 rounded-full bg-gray-100 ">
        <img className="rounded-full  border-green-950 border-2" src={interaction.picture} alt={interaction.name} />
      </div>
    </div>
     </Link>
  );
